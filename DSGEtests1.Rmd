...
## load mat file
```{r function to load results mat file, warning=FALSE, message=FALSE}
...
```{r load results mat file, warning=FALSE, message=FALSE}
...
get_table_1_moments <- function(file_path, n_vars = 18, n_periods = 115, n_reps = 50, base_var_idx = 12) {
  # Base Var is log output, needed for table replication (correlation with output)
  
  # Import the Binary file where data is stored after using simul_replic in Dynare
  # Dynare binary files are double-precision (8 bytes)
  file_size <- file.info(file_path)$size
  raw_data <- readBin(file_path, what = "numeric", n = file_size / 8, size = 8)
  
  # Same reshaping as above: [Variables x Periods x Replications]
  ...
  # 4. Loop through Replications and Variables
  for (i in 1:n_reps) {
    
    # Extract Data for one simulation and apply HP-filter
    # Base var is Variable to show correlation with, in our case Output
    ...
    for (v in 1:n_vars) {
      ...
      # This check to see if there is variation -> checks if data is bad or missing
      if(sd(raw_series) > 1e-10) {
        target_cycle <- hpfilter(raw_series, freq = 1600)$cycle
        
        # store the sd and correlation of individual simulations
  ...
  # Define Variable Names -> Variables always ordered same way, Logs needed by us are at the end
  var_names_clean <- c(
  ...
  final_table <- data.frame(
    Variable        = var_names_clean, # Set names
    Mean_SD         = rowMeans(sd_matrix, na.rm = TRUE), # Take mean of sd (reported value)
    SD_of_SD        = apply(sd_matrix, 1, sd, na.rm = TRUE), # Calculate sd of the sd (reported in parentheses)
    Mean_Corr_Base  = rowMeans(corr_matrix, na.rm = TRUE), # Take mean of correlation with output (reported value)
    SD_of_Corr      = apply(corr_matrix, 1, sd, na.rm = TRUE), # Take sd of correlations (reported in parenteses)
...
# Quick example
results_015_table <- get_table_1_moments(file_path, base_var_idx = 12)
print(results_015_table)
```
