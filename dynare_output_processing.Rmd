...
results_constant_growth <- readMat("models/dmm_g_constant_simul/Output/dmm_g_constant_simul_results.mat")
results_015_growth <- readMat("models/dmm_g_015_simul/Output/dmm_g_015_simul_results.mat")
results_15_growth <- readMat("models/dmm_g_15_simul/Output/dmm_g_15_simul_results.mat")
...
```{r make table 1 with standard dynare method}
...
```{r price specificities}

# Inside the loop for replication 'i'
# 1. Extract log_phat and log_g
log_phat <- sim_data_3d[18, , 1]
log_g    <- log(sim_data_3d[10, , 1]) # Assuming row 'idx_g' is raw g

# 2. Reconstruct Log Money and Log Price
log_m    <- cumsum(log_g)
log_p    <- log_phat + log_m

# 3. Filter and Calculate Correlation
cycle_p  <- hpfilter(log_p, freq = 1600)$cycle
cycle_y  <- hpfilter(sim_data_3d[12, , 1], freq = 1600)$cycle

corr_p_y <- cor(cycle_p, cycle_y)
...
get_table_1_moments <- function(file_path, n_vars = 18, n_periods = 115, n_reps = 50, base_var_idx = 12) {
  ...
  for (i in 1:n_reps) {
    ...
    for (v in 1:n_vars) {
      ...
      # SPECIAL CASE: Price Level (Variable 18)
      if (v == 18) {
  ...
  # Set Variable Names (binary file doesn't provide it)
  var_names_clean <- c(
